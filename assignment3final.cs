...
namespace P2_11
{
  class BankAccount 
  {
    ...
    string getID()     
    {
     ...
     public double getChecking()        {            return checking_balances;        }   
     public double getSaving()        {            return Saving_balances;        }       
     public double getbalance()        {            return Saving_balances+checking_balances;        }     
     public void setID(string name)        {            id = name;        }      
     public void deposit(double depositAmount) 
     {     
      if (depositAmount > 0)    
        checking_balances += depositAmount;    
      }
      public Boolean withdraw(double withdrawAmount)   
      {
        if ( checking_balances>= withdrawAmount && withdrawAmount > 0)      
        {
       ...
       {
        if (checking_balances <= 1000)    
          checking_balances += checking_balances * 0.025;   
         else if (checking_balances < 5000)     
          checking_balances += checking_balances * 0.035;     
         else     
          checking_balances += checking_balances * 0.045;   
        } 
          ...
          static void Main()       
          {
            ...
            while (true)      
            {
              ...
              if (!b2.equals(ID))      
              {
               ...
               Console.WriteLine("理财金额:");     
               Console.WriteLine(b2.getSaving());       
               Console.WriteLine("存储金额:");      
